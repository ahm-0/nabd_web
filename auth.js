(() => {
  ...
  form?.addEventListener('submit', async event => {
    ...
    const data = signupMode ? {
      ...
      gender: form.elements.signup_gender.value,
    ...
    try {
      if (signupMode) {
        const { data: result, error } = await client.auth.signUp({ email: data.email, password: data.password, options: { data: { first_name: data.first_name, father_name: data.father_name, family_name: data.family_name, study_stage: data.study_stage, province: data.province, gender: data.gender } } });
  ...
  async function saveProfile(user, data) {
    const profile = { user_id: user.id, first_name: data.first_name, father_name: data.father_name, family_name: data.family_name, study_stage: data.study_stage, province: data.province, gender: data.gender, email: user.email, daily_usage_minutes: 0, usage_day: new Date().toISOString().slice(0, 10), last_usage_at: new Date().toISOString() };
    let { error } = await client.from('student_profiles').upsert(profile, { onConflict: 'user_id' });
    if (error) {
      ({ error } = await client.from('student_profiles').upsert({ user_id: user.id, first_name: data.first_name, father_name: data.father_name, family_name: data.family_name, study_stage: data.study_stage, province: data.province, gender: data.gender, email: user.email }, { onConflict: 'user_id' }));
    }
    if (error) throw error;
  }
