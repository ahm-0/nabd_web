...
begin
  ...
  return jsonb_build_object(
    ...
    'files', coalesce((select jsonb_agg(jsonb_build_object('id', f.id, 'subject_id', f.subject_id, 'title', f.title, 'description', f.description, 'file_url', f.file_url, 'access_price', f.access_price, 'is_available', f.is_available, 'sort_order', f.sort_order, 'created_at', f.created_at, 'updated_at', f.updated_at) order by f.sort_order, f.title) from public.premium_files f), '[]'::jsonb),
