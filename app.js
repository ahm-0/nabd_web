...
(() => {
  ...
  function filteredUniversities() {
    const query = String($('#universitySearch')?.value || '').trim().toLowerCase(); const province = $('#universityCity')?.value || 'all'; const type = $('#universityType')?.value || 'all'; const entity = $('#universityEntity')?.value || 'all'; const field = $('#universityField')?.value || 'all';
    return universityDirectory.filter(item => (province === 'all' || item.province === province) && (type === 'all' || item.type === type) && (entity === 'all' || item.entity === entity) && (field === 'all' || item.fields.includes(field)) && (!query || `${item.name} ${item.location} ${item.type} ${item.entity} ${item.note} ${item.details} ${item.fields.join(' ')}`.toLowerCase().includes(query)));
  }
  ...
  function renderUniversityDirectory() {
    ...
    if (bar) bar.classList.toggle('hidden', !comparedUniversities.length); if ($('#universityCompareCount')) $('#universityCompareCount').textContent = comparedUniversities.length; if ($('#universityResultsCount')) $('#universityResultsCount').textContent = items.length;
    const selectedProvince = $('#universityCity')?.value || 'all';
    grid.innerHTML = visibleItems.length ? visibleItems.map(item => { const selected = comparedUniversities.includes(item.id); const founded = item.founded ? item.founded : 'غير مذكورة'; return `<article class="university-card panel"><header><span class="university-type ${item.type === 'خاصة' ? 'private' : 'public'}">${escapeHTML(item.type)}</span><label class="compare-toggle"><input type="checkbox" data-university-compare="${escapeHTML(item.id)}" ${selected ? 'checked' : ''}><span>قارن</span></label></header><div class="university-card-title"><span><i class="fa-solid ${item.entity === 'معهد' ? 'fa-graduation-cap' : 'fa-building-columns'}"></i></span><div><h3>${escapeHTML(item.name)}</h3><p><i class="fa-solid fa-location-dot"></i> ${escapeHTML(item.location)} <b>·</b> <span>${escapeHTML(item.entity)}</span></p></div></div><p class="university-note">${escapeHTML(item.note)}</p><div class="university-facts"><span><i class="fa-solid fa-location-dot"></i>${escapeHTML(item.province)}</span><span><i class="fa-regular fa-calendar"></i>${founded}</span></div><div class="university-fields">${item.fields.map(field => `<span>${escapeHTML(field)}</span>`).join('')}</div><div class="university-highlights"><b>مجالات مذكورة</b>${item.highlights.map(highlight => `<span>${escapeHTML(highlight)}</span>`).join('')}</div><button class="outline-button" type="button" data-university-details="${escapeHTML(item.id)}"><i class="fa-solid fa-circle-info"></i> التفاصيل والميزات</button></article>`; }).join('') : `<div class="learning-empty panel"><i class="fa-solid fa-building-columns"></i><h3>لا توجد مؤسسة مذكورة</h3><p>${selectedProvince !== 'all' ? `لا يحتوي الكود المرفق على مؤسسة مسجلة ضمن محافظة ${escapeHTML(selectedProvince)}.` : 'غيّر المحافظة أو النوع أو المجال أو كلمات البحث.'}</p></div>`;
  ...
  function initUniversities() {
    ...
    ['universitySearch', 'universityCity', 'universityType', 'universityEntity', 'universityField'].forEach(id => $(`#${id}`)?.addEventListener(id === 'universitySearch' ? 'input' : 'change', resetUniversityVisibleLimit));
