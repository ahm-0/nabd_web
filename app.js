...
(() => {
  ...
  const universityDirectory = [
    { id: 'damascus', name: 'جامعة دمشق', city: 'دمشق', founded: 1923, type: 'حكومية', fields: ['صحي', 'هندسي', 'علمي', 'إنساني'], highlights: ['طب وعلوم صحية', 'هندسات', 'حقوق واقتصاد', 'آداب ولغات'], advantages: ['تنوع واسع للكليات والتخصصات', 'خيارات تعليم مفتوح معلنة عبر الجامعة', 'مسارات بحث أكاديمي متعددة'], note: 'جامعة حكومية متعددة الكليات في دمشق.', official: 'https://damascusuniversity.edu.sy/' },
    { id: 'aleppo', name: 'جامعة حلب', city: 'حلب', founded: 1958, type: 'حكومية', fields: ['صحي', 'هندسي', 'علمي', 'إنساني'], highlights: ['طب وعلوم صحية', 'هندسات', 'آداب وعلوم'], advantages: ['تنوع بين المجالات النظرية والتطبيقية', 'برامج في المجالات الصحية والهندسية', 'مسارات للعلوم والآداب'], note: 'جامعة حكومية في مدينة حلب.', official: 'http://www.alepuniv.edu.sy/' },
    { id: 'tishreen', name: 'جامعة تشرين', city: 'اللاذقية', founded: 1971, type: 'حكومية', fields: ['صحي', 'هندسي', 'علمي', 'إنساني'], highlights: ['طب وعلوم صحية', 'تقانات وهندسات', 'علوم وتربية'], advantages: ['مجالات علمية وإنسانية متنوعة', 'تخصصات صحية وهندسية للاستكشاف', 'وجود في محافظة اللاذقية'], note: 'جامعة حكومية في اللاذقية.', official: 'http://www.tishreen.edu.sy/' },
    { id: 'baath', name: 'جامعة البعث', city: 'حمص', founded: 1979, type: 'حكومية', fields: ['صحي', 'هندسي', 'علمي', 'إنساني'], highlights: ['طب وعلوم صحية', 'هندسات', 'اقتصاد وآداب'], advantages: ['تشكيلة من الاختصاصات الأكاديمية والتطبيقية', 'مجالات صحية وهندسية', 'موقع في مدينة حمص'], note: 'جامعة حكومية في حمص.', official: 'https://homs-univ.edu.sy/' },
    { id: 'virtual', name: 'الجامعة الافتراضية السورية', city: 'دمشق', founded: 2002, type: 'حكومية', fields: ['تقاني', 'علمي', 'إنساني'], highlights: ['تقانات معلومات', 'إدارة وأعمال', 'برامج مرنة'], advantages: ['نمط دراسة يعتمد على التعليم الافتراضي', 'مجالات تقنية وإدارية متنوعة', 'يتطلب التحقق من نمط البرنامج وشروطه'], note: 'جامعة حكومية تقدم برامج بنمط افتراضي.', official: 'https://svuonline.org/' },
    { id: 'furat', name: 'جامعة الفرات', city: 'دير الزور', founded: 2006, type: 'حكومية', fields: ['هندسي', 'علمي', 'إنساني'], highlights: ['علوم', 'هندسات', 'تربية وآداب'], advantages: ['مسارات محلية متنوعة', 'مجالات علمية وتطبيقية', 'ابحث في الإعلان الرسمي عن البرامج المتاحة'], note: 'جامعة حكومية في دير الزور.', official: 'http://www.alfuratuniv.edu.sy/' },
    { id: 'hama', name: 'جامعة حماة', city: 'حماة', founded: 2014, type: 'حكومية', fields: ['صحي', 'هندسي', 'علمي', 'إنساني'], highlights: ['علوم صحية', 'هندسات', 'اقتصاد وتربية'], advantages: ['مجالات دراسية متعددة', 'موقع في محافظة حماة', 'خيارات أكاديمية وتطبيقية'], note: 'جامعة حكومية في حماة.', official: 'http://www.hama-univ.edu.sy/' },
    { id: 'tartous', name: 'جامعة طرطوس', city: 'طرطوس', founded: 2015, type: 'حكومية', fields: ['صحي', 'هندسي', 'علمي', 'إنساني'], highlights: ['علوم صحية', 'هندسات', 'آداب وعلوم'], advantages: ['موقع في محافظة طرطوس', 'مجالات علمية وإنسانية للاستكشاف', 'تحقق من البرامج المعلنة حاليًا'], note: 'جامعة حكومية في طرطوس.', official: 'http://tartous-univ.edu.sy/' },
    { id: 'aiu', name: 'الجامعة العربية الدولية', city: 'دمشق', founded: 2004, type: 'خاصة', fields: ['صحي', 'هندسي', 'علمي', 'إنساني'], highlights: ['إدارة وأعمال', 'هندسات', 'علوم صحية'], advantages: ['مؤسسة خاصة في دمشق', 'تخصصات إدارية وهندسية وصحية للاستكشاف', 'تحقق من الرسوم والبرامج عبر الجامعة'], note: 'جامعة خاصة في دمشق.', official: 'https://aiu.edu.sy/' }
  ];
  ...
  function filteredUniversities() {
    ...
    return universityDirectory.filter(item => (city === 'all' || item.city === city) && (type === 'all' || item.type === type) && (field === 'all' || item.fields.includes(field)) && (!query || `${item.name} ${item.city} ${item.type} ${item.highlights.join(' ')} ${item.advantages.join(' ')}`.toLowerCase().includes(query)));
  }
  ...
  function openUniversityDetails(id) {
    ...
    openModal(`<div class="modal-head"><div><span class="eyebrow">معلومات إرشادية</span><h3>${escapeHTML(item.name)}</h3></div><button class="close-modal" aria-label="إغلاق">×</button></div><div class="university-modal"><div class="university-detail-facts"><span><i class="fa-solid fa-location-dot"></i>${escapeHTML(item.city)}</span><span><i class="fa-regular fa-calendar"></i>تأسست ${item.founded}</span><span><i class="fa-solid fa-building-columns"></i>${escapeHTML(item.type)}</span></div><p>${escapeHTML(item.note)}</p><h4>مزايا للاستكشاف</h4><ul>${item.advantages.map(advantage => `<li><i class="fa-solid fa-check"></i>${escapeHTML(advantage)}</li>`).join('')}</ul><h4>مجالات وبرامج محتملة</h4><div class="university-fields">${item.highlights.map(highlight => `<span>${escapeHTML(highlight)}</span>`).join('')}</div><div class="modal-actions"><a class="outline-button" href="${escapeHTML(item.official)}" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-arrow-up-right-from-square"></i> الموقع الرسمي</a><button class="primary-button close-modal" type="button"><i class="fa-solid fa-code-compare"></i> أضف للمقارنة من القائمة</button></div></div>`);
  }
  ...
  function openUniversityCompare() {
    ...
    openModal(`<div class="modal-head"><div><span class="eyebrow">مقارنة إرشادية</span><h3>قارن خياراتك</h3></div><button class="close-modal" aria-label="إغلاق">×</button></div><div class="university-compare-table" style="--cols:${items.length + 1}"><div class="compare-row heading"><b>المعيار</b>${items.map(item => `<b>${escapeHTML(item.name)}</b>`).join('')}</div><div class="compare-row"><span>المدينة</span>${items.map(item => `<span>${escapeHTML(item.city)}</span>`).join('')}</div><div class="compare-row"><span>سنة التأسيس</span>${items.map(item => `<span>${item.founded}</span>`).join('')}</div><div class="compare-row"><span>النوع</span>${items.map(item => `<span>${escapeHTML(item.type)}</span>`).join('')}</div><div class="compare-row"><span>المجالات</span>${items.map(item => `<span>${escapeHTML(item.fields.join(' · '))}</span>`).join('')}</div><div class="compare-row"><span>محاور الاستكشاف</span>${items.map(item => `<span>${escapeHTML(item.highlights.join(' · '))}</span>`).join('')}</div></div><p class="comparison-disclaimer"><i class="fa-solid fa-circle-info"></i> لا تمثل المقارنة ترتيبًا أو أفضلية مهنية أو قبولًا. راجع البرامج والمواعيد والرسوم عبر المصدر الرسمي.</p>`);
  }
