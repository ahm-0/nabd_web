...
(() => {
  ...
  chatMessages = Array.isArray(chatMessages) ? chatMessages.slice(-CHAT_PAGE_SIZE) : [];
  ...
  function saveChatState() {
    try { localStorage.setItem(STORE + 'student_chat_messages', JSON.stringify(chatMessages.slice(-CHAT_PAGE_SIZE))); localStorage.setItem(STORE + 'student_chat_settings', JSON.stringify(chatSettings)); return true; }
  ...
  async function initChatPage() {
    ...
    messages?.addEventListener('pointerdown', event => { const row = event.target.closest('[data-chat-message]'); if (!row) return; startX = event.clientX; activeRow = row; longPressTriggered = false; row.setPointerCapture?.(event.pointerId); row.style.setProperty('--swipe-x', '0px'); holdTimer = window.setTimeout(() => { longPressTriggered = true; showChatActionMenu(row.dataset.chatMessage, row); }, 550); });
    messages?.addEventListener('pointermove', event => { if (!activeRow || longPressTriggered) return; const distance = event.clientX - startX; if (Math.abs(distance) < 4) return; window.clearTimeout(holdTimer); activeRow.classList.add('is-swiping'); activeRow.dataset.swipeDirection = distance > 0 ? 'right' : 'left'; activeRow.style.setProperty('--swipe-x', `${Math.max(-64, Math.min(64, distance * 0.55))}px`); });
    messages?.addEventListener('pointerup', event => { if (!activeRow) return; window.clearTimeout(holdTimer); const row = activeRow; const distance = event.clientX - startX; const wasLongPress = longPressTriggered; activeRow = null; row.releasePointerCapture?.(event.pointerId); if (!wasLongPress && Math.abs(distance) >= 52) setChatReply(row.dataset.chatMessage); row.classList.remove('is-swiping'); row.style.setProperty('--swipe-x', '0px'); delete row.dataset.swipeDirection; });
