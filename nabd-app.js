...
(() => {
  ...
  async function sendLocalChatMessage(form) {
    ...
    const now = Date.now(); const message = { id: `chat-${now}`, sender: 'student', name: fullName(), text, createdAt: now, replyTo: chatReplyTarget ? { text: chatReplyTarget.text, name: chatReplyTarget.name } : null }; chatMessages.push(message); saveChatState(); input.value = ''; chatReplyTarget = null; clearChatReply(); renderChatPage();
