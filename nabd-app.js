...
(() => {
  ...
  async function savePublicChatMessage(message) {
    if (!supabaseClient || !currentAuthUser?.id) return;
    try { await supabaseClient.from('public_chat_messages').insert({ sender_id: currentAuthUser.id, body: message.text, reaction: message.reaction || null }); } catch (error) { console.warn('تعذر مزامنة رسالة الدردشة العامة', error); }
  }
  ...
  function sendLocalChatMessage(form) {
    ...
    const now = Date.now(); const message = { id: `chat-${now}`, sender: 'student', name: fullName(), text, createdAt: now, replyTo: chatReplyTarget ? { text: chatReplyTarget.text, name: chatReplyTarget.name } : null }; chatMessages.push(message); saveChatState(); void savePublicChatMessage(message); input.value = ''; chatReplyTarget = null; clearChatReply(); renderChatPage();
  }
  ...
  function setChatReaction(reaction) { const message = chatMessages.find(item => item.id === activeChatMessageId) || remotePublicChatMessages.find(item => item.id === activeChatMessageId); if (!message) return; message.reaction = reaction; message.reacted = true; if (message.sender === 'student') { saveChatState(); if (message.remoteId && supabaseClient) void supabaseClient.from('public_chat_messages').update({ reaction }).eq('id', message.remoteId).eq('sender_id', currentAuthUser?.id); } else { const reactions = readStorage('chat_message_reactions', {}); reactions[message.remoteId || message.id] = reaction; try { localStorage.setItem(STORE + 'chat_message_reactions', JSON.stringify(reactions)); } catch {} } $('#chatReactionMenu')?.classList.remove('show'); activeChatMessageId = null; renderChatPage(); }
