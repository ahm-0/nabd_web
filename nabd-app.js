...
(() => {
  ...
  function showChatActionMenu(id, anchor) { activeChatMessageId = id; const menu = $('#chatActionMenu'); if (!menu) return; const message = chatMessages.find(item => item.id === id) || remotePublicChatMessages.find(item => item.id === id); const own = message?.sender === 'student'; $('[data-chat-edit]', menu)?.classList.toggle('hidden', !own); $('[data-chat-delete]', menu)?.classList.toggle('hidden', !(own || (chatCanModerate && message?.remoteId))); const rect = anchor.getBoundingClientRect(); menu.style.top = `${Math.min(window.innerHeight - 105, rect.bottom + 6)}px`; menu.style.left = `${Math.max(8, Math.min(window.innerWidth - 170, rect.left))}px`; menu.classList.add('show'); }
