...
(() => {
  ...
  async function loadPrivateConversations() { if (!supabaseClient || !currentAuthUser?.id) return; const result = await supabaseClient.rpc('chat_list_private_conversations'); if (result.error) { console.warn('تعذر تحميل قائمة المحادثات الخاصة', result.error); return; } privateConversations = result.data || []; const badge = $('#chatConversationsRequestsBadge'); if (badge) badge.textContent = String(chatRequests.length); renderPrivateConversations(); }
