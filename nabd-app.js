...
(() => {
  ...
  function toggleChatReaction(id, button) { const message = chatMessages.find(item => item.id === id); const menu = $('#chatReactionMenu'); if (!message || !menu || !button) return; activeChatMessageId = id; const rect = button.getBoundingClientRect(); menu.style.top = `${Math.max(8, rect.top - 48)}px`; menu.style.left = `${Math.max(8, Math.min(window.innerWidth - 240, rect.left - 100))}px`; menu.classList.add('show'); }
  function setChatReaction(reaction) { const message = chatMessages.find(item => item.id === activeChatMessageId); if (!message) return; message.reaction = reaction; message.reacted = true; saveChatState(); $('#chatReactionMenu')?.classList.remove('show'); activeChatMessageId = null; renderChatPage(); }
