...
(() => {
  ...
  function renderChatPage() {
    ...
    const studentAvatar = student.avatar ? `<img src="${escapeHTML(student.avatar)}" alt="صورة ${escapeHTML(name)}">` : escapeHTML(initials()); const platformAvatar = '<img src="assets/nabd-logo.jpg" alt="شعار نبض التفوق">';
    holder.innerHTML = chatMessages.length ? chatMessages.map(message => { const own = message.sender === 'student'; const avatar = own ? studentAvatar : platformAvatar; const displayName = message.name || (own ? name : 'نبض التفوق'); return `<article class="chat-bubble-row ${own ? 'mine' : 'other'}" data-chat-message="${escapeHTML(message.id)}"><span class="chat-avatar">${avatar}</span><div class="chat-bubble"><b class="chat-message-name">${escapeHTML(displayName)}</b>${message.replyTo ? `<div class="chat-message-reply">↩ ${escapeHTML(message.replyTo.text)}</div>` : ''}<p>${escapeHTML(message.text)}</p><div class="chat-bubble-meta"><small>${displayAdminDate(message.createdAt)}${message.edited ? ' · معدّلة' : ''}</small>${message.reaction ? `<span class="chat-reaction-badge">${escapeHTML(message.reaction)}</span>` : ''}<button type="button" class="chat-react ${message.reaction ? 'is-reacted' : ''}" data-chat-react="${escapeHTML(message.id)}" aria-label="التفاعلات"><i class="fa-${message.reaction ? 'solid' : 'regular'} fa-heart"></i></button></div></div></article>`; }).join('') : '<div class="chat-empty"><i class="fa-regular fa-comments"></i><b>ابدأ محادثتك</b><span>اسحب الرسالة للرد أو اضغط عليها للتعديل والحذف.</span></div>';
  ...
  function initChatPage() {
    ...
    document.addEventListener('click', event => { if (!event.target.closest('#chatActionMenu, [data-chat-message]')) closeChatActionMenu(); if (!event.target.closest('#chatReactionMenu, [data-chat-react]')) $('#chatReactionMenu')?.classList.remove('show'); });
