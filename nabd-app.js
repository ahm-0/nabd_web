...
(() => {
  ...
  async function loadPublicChat() {
    ...
    try {
      ...
      const remoteRows = data || []; const remoteById = new Map(remoteRows.map(item => [item.id, item]));
  ...
  async function savePublicChatMessage(message) {
    ...
    const result = await supabaseClient.from('public_chat_messages').insert({ sender_id: currentAuthUser.id, body: message.text, reply_to_id: message.replyTo?.remoteId || null, reaction: message.reaction || null });
