...
(() => {
  ...
  let chatSettings = { notifications: true, bio: '', private_chat_enabled: true, ...readStorage('student_chat_settings', {}) };
  let chatStats = { total: 1, online: 1 };
  let chatCanModerate = false;
  let pinnedChatMessage = null;
  let activeChatMessageId = null;
  let chatReplyTarget = null;
  let remotePublicChatMessages = [];
  let chatProfiles = new Map();
  let chatProfileTarget = null;
  let privateThread = null;
  ...
  function toggleSavedItem(item) {
    const id = savedItemId(item); if (!id) return false;
    const exists = isSavedItem(id); savedItems = exists ? savedItems.filter(entry => entry.id !== id) : [{ id, title: String(item.title || 'عنصر محفوظ'), subtitle: String(item.subtitle || ''), type: String(item.type || 'محتوى'), icon: String(item.icon || 'fa-bookmark'), href: String(item.href || ''), meta: item.meta && typeof item.meta === 'object' ? item.meta : {}, createdAt: Date.now() }, ...savedItems];
    if (!saveSavedItems()) return exists;
    toast(exists ? 'تمت إزالة العنصر من المحفوظات.' : 'تمت إضافة العنصر إلى المحفوظات.'); return !exists;
  }
  function savedToggleButtonMarkup(item, extraClass = '') {
    const saved = isSavedItem(item.id); return `<button class="save-item-button ${saved ? 'saved' : ''} ${extraClass}" type="button" data-saved-toggle data-saved-id="${escapeHTML(String(item.id))}" data-saved-title="${escapeHTML(String(item.title || 'عنصر محفوظ'))}" data-saved-subtitle="${escapeHTML(String(item.subtitle || ''))}" data-saved-type="${escapeHTML(String(item.type || 'محتوى'))}" data-saved-icon="${escapeHTML(String(item.icon || 'fa-bookmark'))}" data-saved-href="${escapeHTML(String(item.href || ''))}" aria-pressed="${saved}" title="${saved ? 'إزالة من المحفوظات' : 'حفظ في المحفوظات'}"><i class="${saved ? 'fa-solid' : 'fa-regular'} fa-bookmark"></i><span>${saved ? 'محفوظ' : 'حفظ'}</span></button>`;
  }
  function updateSavedToggleButton(button) {
    const saved = isSavedItem(button.dataset.savedId); button.classList.toggle('saved', saved); button.setAttribute('aria-pressed', String(saved)); button.title = saved ? 'إزالة من المحفوظات' : 'حفظ في المحفوظات'; const icon = $('i', button); const label = $('span', button); if (icon) icon.className = `${saved ? 'fa-solid' : 'fa-regular'} fa-bookmark`; if (label) label.textContent = saved ? 'محفوظ' : 'حفظ';
  }
  let activeSavedFilter = 'all';
  function renderSavedItems() {
    const grid = $('#savedItemsGrid'); const empty = $('#savedItemsEmpty'); if (!grid || !empty) return;
    const query = String($('#savedItemsSearch')?.value || '').trim().toLowerCase(); const filtered = savedItems.filter(item => (activeSavedFilter === 'all' || item.type === activeSavedFilter) && `${item.title} ${item.subtitle} ${item.type}`.toLowerCase().includes(query));
    grid.innerHTML = filtered.map(item => { const href = String(item.href || '').trim(); const open = href ? `<a class="saved-item-open" href="${escapeHTML(href)}"><i class="fa-solid fa-arrow-left"></i> فتح</a>` : ''; return `<article class="saved-item-card panel"><span class="saved-item-icon"><i class="fa-solid ${escapeHTML(item.icon || 'fa-bookmark')}"></i></span><div class="saved-item-copy"><span>${escapeHTML(item.type || 'محتوى')}</span><h3>${escapeHTML(item.title)}</h3><p>${escapeHTML(item.subtitle || 'عنصر محفوظ في منصة نبض التفوق')}</p></div><footer>${open}${savedToggleButtonMarkup(item, 'compact')}</footer></article>`; }).join('');
    const noResults = !filtered.length; grid.classList.toggle('hidden', noResults); empty.classList.toggle('hidden', !noResults); const emptyTitle = empty.querySelector('h3'); const emptyText = empty.querySelector('p'); if (emptyTitle && emptyText) { if (savedItems.length && (query || activeSavedFilter !== 'all')) { emptyTitle.textContent = 'لا توجد نتائج مطابقة'; emptyText.textContent = 'غيّر كلمة البحث أو مرشح المحفوظات للعثور على عناصر أخرى.'; } else { emptyTitle.textContent = 'لا توجد محفوظات بعد'; emptyText.textContent = 'اضغط زر الحفظ في أي برنامج أو كتاب أو اختبار أو جامعة لتجده هنا بسرعة.'; } }
  }
  function initSavedItems() {
    if (!$('#savedItemsGrid')) return; renderSavedItems(); $('#savedItemsSearch')?.addEventListener('input', renderSavedItems); $$('#savedItemsFilters [data-saved-filter]').forEach(button => button.addEventListener('click', () => { activeSavedFilter = button.dataset.savedFilter; $$('#savedItemsFilters [data-saved-filter]').forEach(item => item.classList.toggle('active', item === button)); renderSavedItems(); }));
  }
  ...
  const completionShareFooter = `قم بالانضمام لمنصة نبض التفوق التعليمي\n\n- تطبيقنا يقدم 🔥👇🏻\n  📚 تطبيق شامل للملازم والاختبارات\n  ✓ عدادات تنازلية للامتحانات\n  ✓ نصائح دراسية وخطط يومية\n  ✓ أكثر من 5000 مستخدم يومي!\n\nhttps://whatsapp.com/channel/0029Vb84gXUISTkDgZDFVd2L`;
  async function shareCompletionProgram(track, program) {
    if (!track || !program) return;
    const schedule = (program.entries || []).map(([time, text, duration]) => `${time} — ${text} (${duration})`).join('\n');
    const text = `${program.title}\n${program.subtitle}\n\n${schedule}\n\n${completionShareFooter}`;
    const payload = { title: `${track.label} | ${program.title}`, text, dialogTitle: 'مشاركة برنامج الدراسة' };
    try {
      const nativeShare = capacitorPlugin('Share');
      if (isNativeNabd() && nativeShare?.share) { await nativeShare.share(payload); return; }
      if (navigator.share) { await navigator.share(payload); return; }
      if (navigator.clipboard) { await navigator.clipboard.writeText(text); toast('تم نسخ نص البرنامج للمشاركة.'); return; }
      toast('ميزة المشاركة متاحة عند نشر التطبيق على الهاتف.');
    } catch (error) {
      if (error.name !== 'AbortError') toast('تعذر تنفيذ المشاركة حاليًا.');
    }
  }
  ...
  function renderBaccalaureateSchedule() {
    ...
    const saveId = `completion-program-${activeCompletionTrack}-${activeCompletionProgram}`; const saved = isSavedItem(saveId);
    holder.dataset.tone = track.tone; holder.innerHTML = `<header class="completion-schedule-head"><div><span class="completion-schedule-icon"><i class="fa-solid ${program.icon}"></i></span><div><span>${escapeHTML(track.label)}</span><h4>${escapeHTML(program.title)}</h4><p>${escapeHTML(program.subtitle)}</p></div></div><div class="completion-schedule-tools"><span class="completion-schedule-count">${program.entries.length} محطة</span><button class="completion-schedule-action" type="button" data-completion-share title="مشاركة النص"><i class="fa-solid fa-arrow-up-from-bracket"></i><span>مشاركة</span></button><button class="completion-schedule-action ${saved ? 'saved' : ''}" type="button" data-completion-save title="${saved ? 'إزالة من المحفوظات' : 'حفظ في المحفوظات'}"><i class="${saved ? 'fa-solid' : 'fa-regular'} fa-bookmark"></i><span>${saved ? 'محفوظ' : 'حفظ'}</span></button></div></header><nav class="completion-program-tabs" role="tablist" aria-label="اختيار نمط الدوام">${tabs}</nav><div class="completion-schedule-list">${entries}</div>`;
  }
  function initCompletion() {
    ...
    $('#completionSchedule')?.addEventListener('click', event => { const track = baccalaureatePrograms[activeCompletionTrack]; const program = track?.programs?.[activeCompletionProgram]; if (!track || !program) return; if (event.target.closest('[data-completion-share]')) { shareCompletionProgram(track, program); return; } if (event.target.closest('[data-completion-save]')) { toggleSavedItem({ id: `completion-program-${activeCompletionTrack}-${activeCompletionProgram}`, title: `${track.label} — ${program.title}`, subtitle: program.subtitle, type: 'برنامج ختم المنهاج', icon: program.icon, href: 'completion-program.html', meta: { track: activeCompletionTrack, program: activeCompletionProgram } }); renderBaccalaureateSchedule(); return; } const tab = event.target.closest('[data-completion-program]'); if (!tab) return; activeCompletionProgram = tab.dataset.completionProgram; renderBaccalaureateSchedule(); });
  }
  ...
  async function loadPublicChat() {
    ...
    try {
      let result = await supabaseClient.from('public_chat_messages').select('id,sender_id,body,reply_to_id,reaction,edited_at,created_at,pinned_at,pinned_by').order('created_at', { ascending: true }).limit(120);
      if (result.error) result = await supabaseClient.from('public_chat_messages').select('id,sender_id,body,reply_to_id,reaction,edited_at,created_at').order('created_at', { ascending: true }).limit(120);
      const { data, error } = result;
      if (error) throw error;
      const ids = [...new Set((data || []).map(item => item.sender_id).filter(Boolean))];
      if (ids.length) { const profiles = await supabaseClient.from('student_profiles').select('user_id,first_name,father_name,family_name,study_stage,province,avatar_url,bio').in('user_id', ids); (profiles.data || []).forEach(profile => chatProfiles.set(profile.user_id, profile)); }
      remotePublicChatMessages = (data || []).map(item => { const profile = chatProfiles.get(item.sender_id) || {}; const messageName = `${profile.first_name || ''} ${profile.father_name || ''} ${profile.family_name || ''}`.replace(/\s+/g, ' ').trim(); return { id: `remote-${item.id}`, remoteId: item.id, sender: item.sender_id === currentAuthUser.id ? 'student' : 'other', senderId: item.sender_id, name: messageName || 'طالب نبض التفوق', text: item.body, createdAt: item.created_at, reaction: item.reaction || '', edited: Boolean(item.edited_at), pinned: Boolean(item.pinned_at), replyTo: null, avatar: profile.avatar_url || '', stage: profile.study_stage || '', province: profile.province || '' }; });
  ...
  async function savePublicChatMessage(message) {
    ...
    const result = await supabaseClient.from('public_chat_messages').insert({ sender_id: currentAuthUser.id, body: message.text, reaction: message.reaction || null });
    if (result.error) { console.warn('تعذر مزامنة رسالة الدردشة العامة', result.error); toast(result.error.message || 'تعذر إرسال الرسالة.'); return false; }
    return true;
  }
  async function loadChatRequests() {
    ...
    if (error) return console.warn('تعذر تحميل طلبات الدردشة', error);
    chatRequests = data || []; const senderIds = chatRequests.map(request => request.sender_id).filter(id => !chatProfiles.has(id)); if (senderIds.length) { const profiles = await supabaseClient.from('student_profiles').select('user_id,first_name,father_name,family_name,study_stage,province,avatar_url,bio').in('user_id', senderIds); (profiles.data || []).forEach(profile => chatProfiles.set(profile.user_id, profile)); } const badge = $('#chatRequestsBadge'); if (badge) badge.textContent = String(chatRequests.length); renderChatRequests();
  }
  function renderChatRequests() { const holder = $('#chatRequestsList'); if (!holder) return; holder.innerHTML = chatRequests.length ? chatRequests.map(request => { const profile = chatProfiles.get(request.sender_id) || {}; const name = `${profile.first_name || ''} ${profile.father_name || ''} ${profile.family_name || ''}`.replace(/\s+/g, ' ').trim() || 'طالب نبض التفوق'; return `<article class="chat-request-item"><span class="chat-avatar">${profile.avatar_url ? `<img src="${escapeHTML(profile.avatar_url)}" alt="">` : escapeHTML(name.slice(0, 2))}</span><div><b>${escapeHTML(name)}</b><small>${escapeHTML(profile.study_stage || 'طالب')} · يريد بدء دردشة خاصة</small></div><button type="button" data-chat-request="${escapeHTML(request.id)}" data-chat-request-accept="true">قبول</button><button type="button" data-chat-request="${escapeHTML(request.id)}" data-chat-request-accept="false">رفض</button></article>`; }).join('') : '<div class="chat-empty"><i class="fa-regular fa-bell-slash"></i><span>لا توجد طلبات جديدة.</span></div>'; }
  async function loadChatSettings() { if (!supabaseClient || !currentAuthUser?.id) return; const { data } = await supabaseClient.from('chat_settings').select('private_chat_enabled,chat_notifications_enabled').eq('user_id', currentAuthUser.id).maybeSingle(); if (data) { chatSettings.private_chat_enabled = data.private_chat_enabled !== false; chatSettings.notifications = data.chat_notifications_enabled !== false; saveChatState(); const privateSwitch = $('#chatPrivateSwitch'); const noticeSwitch = $('#chatNoticeSwitch'); if (privateSwitch) privateSwitch.checked = chatSettings.private_chat_enabled; if (noticeSwitch) noticeSwitch.checked = chatSettings.notifications; } }
  function renderChatPage() {
    ...
    const allMessages = [...chatMessages, ...remotePublicChatMessages.filter(remote => !chatMessages.some(local => local.remoteId === remote.remoteId))].sort((a, b) => Number(new Date(a.createdAt)) - Number(new Date(b.createdAt))); const studentAvatar = student.avatar ? `<img src="${escapeHTML(student.avatar)}" alt="صورة ${escapeHTML(name)}">` : escapeHTML(initials());
  ...
  async function sendLocalChatMessage(form) {
    const input = $('[name="chatMessage"]', form); const text = String(input?.value || '').trim(); if (!text) return;
    const now = Date.now(); const senderId = currentAuthUser?.id || ''; const message = { id: `chat-${now}`, sender: 'student', senderId, name: fullName(), avatar: student.avatar || '', stage: student.stage || '', province: student.province || '', text, createdAt: now, replyTo: chatReplyTarget ? { text: chatReplyTarget.text, name: chatReplyTarget.name } : null }; if (senderId) chatProfiles.set(senderId, { user_id: senderId, first_name: student.first || '', father_name: student.father || '', family_name: student.last || '', study_stage: student.stage || '', province: student.province || '', avatar_url: student.avatar || '', bio: student.bio || '' }); chatMessages.push(message); saveChatState(); input.value = ''; chatReplyTarget = null; clearChatReply(); renderChatPage();
    if (!(await savePublicChatMessage(message))) { chatMessages = chatMessages.filter(item => item.id !== message.id); saveChatState(); renderChatPage(); }
  }
  ...
  function showChatActionMenu(id, anchor) { activeChatMessageId = id; const menu = $('#chatActionMenu'); if (!menu) return; const message = chatMessages.find(item => item.id === id) || remotePublicChatMessages.find(item => item.id === id); const own = message?.sender === 'student'; $('[data-chat-edit]', menu)?.classList.toggle('hidden', !own); $('[data-chat-delete]', menu)?.classList.toggle('hidden', !(own || (chatCanModerate && message?.remoteId))); const rect = anchor.getBoundingClientRect(); menu.style.top = `${Math.min(window.innerHeight - 150, rect.bottom + 6)}px`; menu.style.left = `${Math.max(8, Math.min(window.innerWidth - 170, rect.left))}px`; menu.classList.add('show'); }
  ...
  async function deleteChatMessage() { const id = activeChatMessageId; const message = chatMessages.find(item => item.id === id) || remotePublicChatMessages.find(item => item.id === id); closeChatActionMenu(); if (!id || !window.confirm('حذف هذه الرسالة؟')) return; if (message?.remoteId && chatCanModerate && supabaseClient) { const result = await supabaseClient.rpc('chat_moderate_delete_message', { p_message: message.remoteId }); if (result.error) return toast(result.error.message || 'تعذر حذف الرسالة.'); remotePublicChatMessages = remotePublicChatMessages.filter(item => item.remoteId !== message.remoteId); renderChatPage(); toast('تم حذف الرسالة من الدردشة.'); return; } chatMessages = chatMessages.filter(item => item.id !== id); saveChatState(); renderChatPage(); toast('تم حذف الرسالة.'); }
  ...
  function startChatRealtime() { if (!supabaseClient || !currentAuthUser?.id || chatRealtimeChannel) return; chatRealtimeChannel = supabaseClient.channel('public-chat-live').on('postgres_changes', { event: '*', schema: 'public', table: 'public_chat_messages' }, () => { void loadPublicChat(); }).subscribe(); }
  async function refreshChatAfterSession() {
    ...
    chatRefreshInFlight = (async () => {
      ...
      await Promise.allSettled([loadChatStats(), loadChatPlatformSettings(), loadChatSettings(), loadPublicChat(), loadChatRequests()]);
      startChatRealtime();
      renderChatPage();
      setSectionLoading('.chat-main', false);
  ...
  async function initChatPage() {
    ...
    $('#chatActionMenu [data-chat-edit]')?.addEventListener('click', editChatMessage); $('#chatActionMenu [data-chat-delete]')?.addEventListener('click', deleteChatMessage); $$('#chatActionMenu [data-chat-reaction-option]').forEach(button => button.addEventListener('click', () => setChatReaction(button.dataset.chatReactionOption)));
    const messages = $('#chatMessages'); let startX = 0; let activeRow = null; let holdTimer = null;
    let longPressTriggered = false;
    messages?.addEventListener('pointerdown', event => { const row = event.target.closest('[data-chat-message]'); if (!row) return; startX = event.clientX; activeRow = row; longPressTriggered = false; holdTimer = window.setTimeout(() => { longPressTriggered = true; showChatActionMenu(row.dataset.chatMessage, row); }, 550); });
    messages?.addEventListener('pointerup', event => { if (!activeRow) return; window.clearTimeout(holdTimer); const row = activeRow; const distance = event.clientX - startX; const wasLongPress = longPressTriggered; activeRow = null; if (!wasLongPress && Math.abs(distance) >= 42) { setChatReply(row.dataset.chatMessage); row.classList.remove('is-swiping'); } });
    messages?.addEventListener('pointercancel', () => { window.clearTimeout(holdTimer); activeRow = null; longPressTriggered = false; });
    ...
    $('#chatNoticeSwitch')?.addEventListener('change', async event => { chatSettings.notifications = event.target.checked; saveChatState(); if (supabaseClient && currentAuthUser?.id) await supabaseClient.from('chat_settings').upsert({ user_id: currentAuthUser.id, private_chat_enabled: chatSettings.private_chat_enabled !== false, chat_notifications_enabled: event.target.checked, updated_at: new Date().toISOString() }); toast(event.target.checked ? 'تم تفعيل إشعارات الدردشة.' : 'تم إيقاف إشعارات الدردشة.'); });
  ...
  function openSupportReply(ticketId) {
    ...
    const messages = supportThread(ticket).map(message => `<article class="admin-support-bubble ${message.sender === 'admin' ? 'admin' : 'student'}" data-support-message-id="${escapeHTML(message.id || '')}" data-support-message-text="${escapeHTML(message.text || '')}" data-support-message-sender="${message.sender === 'admin' ? 'admin' : 'student'}" tabindex="0" role="button" aria-label="اسحب للرد على الرسالة"><div><b>${message.sender === 'admin' ? 'فريق نبض التفوق' : escapeHTML(ticket.name || 'الطالب')}</b><p>${escapeHTML(message.text)}</p><small>${displayAdminDate(message.createdAt || message.created_at || ticket.createdAt || ticket.created_at)} <span class="admin-support-swipe-hint">اسحب للرد</span></small></div></article>`).join('');
    openModal(`<div class="admin-support-chat-modal"><header class="modal-head admin-support-chat-modal-head"><div class="admin-support-chat-modal-head"><span class="admin-support-chat-modal-avatar"><i class="fa-solid fa-headset"></i></span><div class="admin-support-chat-modal-copy"><h3>${escapeHTML(ticket.name || 'طالب نبض')}</h3><small><i class="fa-solid fa-circle"></i> محادثة الدعم</small></div></div><button class="close-modal" aria-label="إغلاق">×</button></header><section class="admin-support-thread admin-support-live-chat" aria-live="polite">${messages || '<p class="sheet-hint">لا توجد رسائل بعد.</p>'}</section><form class="admin-support-chat-composer" data-ticket-id="${escapeHTML(ticket.id)}"><div class="admin-support-quote hidden" data-support-quote><div><i class="fa-solid fa-reply"></i><span><b data-support-quote-name>الرد على الرسالة</b><small data-support-quote-text></small></span></div><button type="button" data-clear-support-quote aria-label="إلغاء الاقتباس">×</button></div><div class="admin-support-reply-row"><textarea name="reply" required maxlength="700" placeholder="اكتب ردًا للطالب..." autofocus></textarea><button type="submit" class="admin-support-send" aria-label="إرسال الرد" title="إرسال الرد"><i class="fa-solid fa-paper-plane"></i></button></div><div class="form-actions"><button type="button" class="outline-button close-modal">إغلاق</button><span class="sheet-hint">اسحب أي رسالة للرد عليها.</span></div></form></div>`);
    const modal = $('#modalContent'); const thread = $('.admin-support-live-chat', modal); const form = $('.admin-support-chat-composer', modal); const quote = $('[data-support-quote]', form); const quoteName = $('[data-support-quote-name]', form); const quoteText = $('[data-support-quote-text]', form); const input = $('[name="reply"]', form);
    let startX = 0; let activeBubble = null;
    const setQuote = bubble => { if (!bubble || bubble.dataset.supportMessageSender === 'admin') return; const body = String(bubble.dataset.supportMessageText || '').trim(); if (!body) return; form.dataset.replyQuote = body; form.dataset.replyQuoteName = bubble.querySelector('b')?.textContent || ticket.name || 'الطالب'; quoteName.textContent = `الرد على ${form.dataset.replyQuoteName}`; quoteText.textContent = body; quote.classList.remove('hidden'); input?.focus(); };
    thread?.querySelectorAll('.admin-support-bubble').forEach(bubble => { bubble.addEventListener('pointerdown', event => { startX = event.clientX; activeBubble = bubble; bubble.classList.add('is-swiping'); }); bubble.addEventListener('pointerup', event => { const distance = event.clientX - startX; bubble.classList.remove('is-swiping'); if (Math.abs(distance) >= 42) setQuote(activeBubble); activeBubble = null; }); bubble.addEventListener('pointercancel', () => { bubble.classList.remove('is-swiping'); activeBubble = null; }); bubble.addEventListener('keydown', event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setQuote(bubble); } }); });
    $('[data-clear-support-quote]', form)?.addEventListener('click', () => { delete form.dataset.replyQuote; delete form.dataset.replyQuoteName; quote.classList.add('hidden'); quoteText.textContent = ''; });
    thread?.scrollTo({ top: thread.scrollHeight, behavior: 'auto' });
  }

  async function sendAdminSupportReply(form) {
    const ticketId = form.dataset.ticketId; const ticket = adminSupportThreads.find(item => item.id === ticketId) || supportTickets.find(item => item.id === ticketId); const rawText = String($('[name="reply"]', form)?.value || '').trim(); const quoted = String(form.dataset.replyQuote || '').trim(); const text = quoted ? `↩️ الرد على: ${quoted.slice(0, 180)}\n\n${rawText}` : rawText; if (!ticket || !rawText) return;
  ...
  function handleAdminAction(button) {
    ...
    if (chatModeratorOnly && !['refresh-chat-admin'].includes(action)) return toast('هذه الأداة متاحة لصاحب البوابة فقط.');
  ...
  async function saveChatAdminSettings(form) { const enabled = $('#adminChatEnabled')?.checked !== false; const announcement = String(new FormData(form).get('announcement') || ''); const result = await supabaseClient.rpc('chat_admin_update_settings', { p_enabled: enabled, p_announcement: announcement || null }); if (result.error) return toast(result.error.message || 'تعذر حفظ إعدادات الدردشة.'); toast(enabled ? 'تم تشغيل الدردشة وحفظ الإعدادات.' : 'تم إيقاف الدردشة وحفظ الإعدادات.'); }
  async function toggleChatAdminEnabled(event) { const enabled = event.currentTarget.checked; const result = await supabaseClient.rpc('chat_admin_set_enabled', { p_enabled: enabled }); if (result.error) { event.currentTarget.checked = !enabled; return toast(result.error.message || 'تعذر تغيير حالة الدردشة.'); } toast(enabled ? 'تمت إعادة تشغيل الدردشة.' : 'تم إيقاف الدردشة.'); }
  ...
  async function setChatUserBan(form) { const data = new FormData(form); const userRef = String(data.get('ban_user_ref') || '').trim(); if (!userRef) return toast('أدخل البريد الإلكتروني أو UUID للمستخدم.'); const result = await supabaseClient.rpc('chat_admin_ban_user', { p_user_ref: userRef, p_banned: data.get('ban_action') !== 'unban', p_reason: String(data.get('ban_reason') || ''), p_expires_at: null }); if (result.error) return toast(result.error.message || 'تعذر تحديث حظر المستخدم.'); form.reset(); toast(data.get('ban_action') === 'unban' ? 'تم رفع حظر المستخدم.' : 'تم حظر المستخدم من الدردشة.'); }
  ...
  function bindAdminDashboardControls() {
    ...
    $('#adminChatEnabled')?.addEventListener('change', event => { void toggleChatAdminEnabled(event); });
    $('#adminChatMessageForm')?.addEventListener('submit', event => { event.preventDefault(); void sendChatAdminMessage(event.currentTarget); });
    $('#chatBanForm')?.addEventListener('submit', event => { event.preventDefault(); void setChatUserBan(event.currentTarget); });
  ...
  async function initAdminDashboard() {
    ...
    try {
      const result = await supabaseClient?.rpc('premium_is_admin');
      const chatResult = await supabaseClient?.rpc('chat_can_moderate');
      const isPremiumAdmin = !result?.error && result?.data === true;
      const canModerateChat = !chatResult?.error && chatResult?.data === true;
      remoteAdminVerified = isPremiumAdmin || canModerateChat;
      chatModeratorOnly = !isPremiumAdmin && canModerateChat;
    ...
    if (chatModeratorOnly) {
      $$('#adminTabs [data-admin-tab]').forEach(button => { const allowed = button.dataset.adminTab === 'chat'; button.hidden = !allowed; });
      activeAdminTab = 'chat';
      $('.admin-compact-title .eyebrow')?.replaceChildren(document.createTextNode('مشرف الدردشة'));
      $('#chatModeratorForm')?.closest('.admin-chat-setting-card')?.setAttribute('hidden', '');
    }
    renderAdminDashboard();
  ...
  function bindEvents() {
    document.addEventListener('click', event => {
      ...
      if (pinButton) { event.preventDefault(); void toggleChatPin(pinButton.dataset.chatPin); return; }
      const pinnedBanner = event.target.closest('[data-chat-pinned-scroll]');
      if (pinnedBanner) { event.preventDefault(); scrollToPinnedMessage(pinnedBanner.dataset.chatPinnedScroll); return; }
      const chatReaction = event.target.closest('[data-chat-react]');
      if (chatReaction) { event.preventDefault(); toggleChatReaction(chatReaction.dataset.chatReact, chatReaction); return; }
