...
(() => {
  ...
  function startChatRealtime() { if (!supabaseClient || !currentAuthUser?.id || chatRealtimeChannel) return; chatRealtimeChannel = supabaseClient.channel('chat-live-all').on('postgres_changes', { event: '*', schema: 'public', table: 'public_chat_messages' }, () => { void loadPublicChat(); }).on('postgres_changes', { event: '*', schema: 'public', table: 'chat_platform_settings' }, () => { void loadChatPlatformSettings(); }).on('postgres_changes', { event: '*', schema: 'public', table: 'private_chat_messages' }, payload => { void loadPrivateConversations(); if (privateThread?.id && payload.new?.thread_id === privateThread.id) void loadPrivateChatMessages(); }).on('postgres_changes', { event: '*', schema: 'public', table: 'private_chat_requests', filter: `recipient_id=eq.${currentAuthUser.id}` }, () => { void loadChatRequests(); }).subscribe(); }
