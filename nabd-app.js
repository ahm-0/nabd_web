...
(() => {
  ...
  function setSectionLoading(selector, active, label = 'جارٍ التحميل') { const root = typeof selector === 'string' ? $(selector) : selector; if (!root) return; const custom = root.querySelector(':scope > #chatLoadingState, :scope > #newsLoadingState'); if (custom) { custom.querySelector('span')?.replaceChildren(document.createTextNode(label)); custom.hidden = !active; root.setAttribute('aria-busy', String(Boolean(active))); return; } let indicator = root.querySelector(':scope > .section-loading-indicator'); if (active) { if (!indicator) { indicator = document.createElement('div'); indicator.className = 'section-loading-indicator'; indicator.setAttribute('role', 'status'); root.prepend(indicator); } indicator.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i><span>${escapeHTML(label)}</span>`; indicator.hidden = false; root.setAttribute('aria-busy', 'true'); } else { if (indicator) indicator.remove(); root.removeAttribute('aria-busy'); } }

  async function initNews() {
    if (!$('#feed')) return;
    setSectionLoading('.news-material-main', true, 'جارٍ تحديث الأخبار'); const newsLoader = $('#newsLoadingState'); if (newsLoader) newsLoader.hidden = false;
    elevateNewsComposer();
    const dock = $('#newsComposerDock');
    ...
    if (!supabaseClient) { setSectionLoading('.news-material-main', false); if (newsLoader) newsLoader.hidden = true; return; }
    try {
    ...
    renderFeed();
    setSectionLoading('.news-material-main', false); if (newsLoader) newsLoader.hidden = true;
  }
  ...
  function startChatRealtime() { if (!supabaseClient || !currentAuthUser?.id || chatRealtimeChannel) return; chatRealtimeChannel = supabaseClient.channel('chat-live-all').on('postgres_changes', { event: '*', schema: 'public', table: 'public_chat_messages' }, () => { void loadPublicChat(); }).on('postgres_changes', { event: '*', schema: 'public', table: 'chat_platform_settings' }, () => { void loadChatPlatformSettings(); }).on('postgres_changes', { event: '*', schema: 'public', table: 'private_chat_messages' }, payload => { void loadPrivateConversations(); void loadPrivateUnreadCount(); if (privateThread?.id && payload.new?.thread_id === privateThread.id) { void loadPrivateChatMessages(); void markPrivateThreadRead(); } }).on('postgres_changes', { event: '*', schema: 'public', table: 'private_chat_requests', filter: `recipient_id=eq.${currentAuthUser.id}` }, () => { void loadChatRequests(); }).subscribe(); }
  ...
  async function initChatPage() {
    ...
    messages?.addEventListener('pointercancel', () => { window.clearTimeout(holdTimer); activeRow = null; longPressTriggered = false; }); messages?.addEventListener('dblclick', event => { const row = event.target.closest('[data-chat-message]'); if (!row) return; event.preventDefault(); showChatActionMenu(row.dataset.chatMessage, row, true); });
